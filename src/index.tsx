...
class App extends React.Component<any, any> {
  ...
  showLoaders = (names: [string], callback?: () => void) => {
    const loaders = Object.assign({}, ...names.map((n: string) => ({ [n]: true })));
    this.setState(
  ...
  hideLoaders = (names: [string], callback?: () => void) =>  {
    const loaders = Object.assign({}, ...names.map((n: string) => ({ [n]: false })));
    this.setState(
      (prevState: any) => ({ loaders: { ...prevState.loaders, ...loaders } }),
      callback
    );
  ...
  showModals = (names: [string], callback?: () => void) => {
    const modals = Object.assign({}, ...names.map((n: string) => ({ [n]: true })));
    this.setState(
  ...
  hideModals = (names: [string], callback?: () => void) => {
    const modals = Object.assign({}, ...names.map((n: string) => ({ [n]: false })));
    this.setState(
      (prevState: any) => ({ modals: { ...prevState.modals, ...modals } }),
      callback
    );
  ...
  fetchGithubDetails() {
    ...
    shortFetch(this.state.user.repos_url, { headers })
  ...
  render() {
    ...
    let value: any = {
      setState: (s1: any, callback?: () => void) => (
        this.setState((s: any) => Object.assign({}, s, s1), callback)
      ),
